...
export class ImprimirFactura{
    constructor(){}

    construirFactura(factura: Factura,detalleFactura: FacturaDetalle[],compania:Compania){
       ...
       let docDefinition = {
        content: [
          ...
          columns: [
            [
              {
                text: compania.nombre,
                bold:true
              },
              { text: compania.direccion},
              { text: compania.email},
              { text: compania.telefono }
            ],
            [
              {
                text: `Factura para Comprobante Final`,
                alignment : 'right',
                bold:true,
              },
              { 
                text: `NCF : B010000045225`,
                alignment : 'right',
              },
              { 
                text: `No. Orden : ${((Math.random() *1000).toFixed(0))}`,
                alignment : 'right',
              }
            ]
          ]
          ...
          },
          {
            columns: [
              ...
              [
                ...
                // },
                // { 
                //   text: `No de Orden : ${((Math.random() *1000).toFixed(0))}`,
                //   alignment: 'right'
                // }
              ]
            ]
          ...
          {canvas: [{ type: 'line', x1: 0, y1: 5, x2: 595-2*40, y2: 5, lineWidth: 3 }]},
          {
            table: {
              ...
              body: [
                [{text:'Producto',  bold: true} ,{text: 'Precio',bold: true}, {text:'Cant.',bold: true},{text:'Desc.',bold: true},{text:'ITBIS',bold: true}, {text:'Total',bold: true}],
                ...detalleFactura.map(p => ([p.articulo, p.precio.toFixed(2), p.cantidad.toFixed(2),p.totalDescuento.toFixed(2),p.totalImpuesto.toFixed(2), p.totalNeto.toFixed(2)])),
              ]
            },
            layout: tableLayout,
          },
            ...
            columns: [
              {},
              {
                table: {
                  ...
                  body: [
                    [
                      { text: "SubTotal", style: ["td2", "strong"] },
                      {
                        text: factura.subtotal.toFixed(2),
                        alignment: "right",
                        style: "td2"
                      }
                    ],
                    [
                      { text: "Desc.", style: ["td2", "strong"] },
                      { text: factura.descuentoTotal.toFixed(2), alignment: "right", style: "td2" }
                    ],
                    [
                      { text: "Imp. (18%)", style: ["td2", "strong"] },
                      { text: factura.impuestoTotal.toFixed(2), alignment: "right", style: "td2" }
                    ],
                    [
                      { text: "Total (DOP)", style: ["td2", "strong"] },
                      {
                        text: factura.totalNeto.toFixed(2),
                        alignment: "right",
                        style: ["td2", "strong"]
                      }
                    ]
         ...
         
          {
            columns: [
              [
                ...
                { text: 'Gerente de Venta', alignment: 'left', italics: true}
              ],
              [
                { text: '____________________________', alignment: 'center', },
                { text: 'Gerente de General', alignment: 'center', italics: true}
              ],
              [
                { text: '____________________________', alignment: 'right', },
                { text: 'Miembro del consejo', alignment: 'right', italics: true}
              ],
            ]
          },
          "\n",
          ...
          {
            text: 'Ahi va toda la nota de la factura.',
          },
          "\n",
          {
            columns: [
              [{ qr: `${factura.nombreCliente}`, fit: '50', alignment: 'center' }],
